...
function checkGameWin() {
    ...
    if (matchedCount === 6) { // 3 pairs * 2
        const feedbackGame = document.getElementById('feedback-game');
        feedbackGame.innerHTML = '🎉 ¡Excelente! Has relacionado todos los conceptos correctamente.';
        feedbackGame.style.display = 'block';
...
function checkStep(stepNum, totalSteps, feedbackId) {
    ...
    if (stepNum === currentStep) {
        ...
        numCircle.style.background = '#28a745';
        currentStep++;
        
        if (currentStep > totalSteps) {
            feedback.innerHTML = '✨ ¡Perfecto! Ese es el orden correcto para parafrasear.';
            feedback.style.display = 'block';
            feedback.className = 'feedback-box feedback-correct';
        }
    } else {
        feedback.innerHTML = '❌ Ese no es el siguiente paso. ¡Intenta de nuevo!';
        feedback.style.display = 'block';
...
function resetOrdering(totalSteps, feedbackId) {
    ...
    document.querySelectorAll('.order-item').forEach(item => {
        ...
        const numCircle = item.querySelector('.order-number');
        numCircle.style.background = '#244b5a';
    });
